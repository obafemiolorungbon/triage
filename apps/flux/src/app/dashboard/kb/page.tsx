...
export default function KnowledgeBasePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {importOpen && (
        <div className="fixed inset-0 z-50 flex justify-end bg-paper-950/75 backdrop-blur-sm">
          <button
            type="button"
          ...
          />
          <aside
            className="surface relative h-full w-full max-w-3xl overflow-y-auto rounded-none border-l border-paper-100/10 p-5 shadow-2xl md:p-6"
            aria-modal="true"
            role="dialog"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-paper-400">
                  Intercom Articles, Freshdesk Solutions, or Zendesk Guide JSON.
                </p>
              </div>
              <button
                type="button"
                className="rounded-lg bg-paper-100/[0.06] p-2 text-paper-300 transition-colors hover:bg-paper-100/[0.1] hover:text-paper-50"
                aria-label="Close import drawer"
                onClick={() => setImportOpen(false)}
              >
                <CloseIcon />
            ...
            <div className="mt-5 grid grid-cols-3 gap-1 rounded-lg bg-paper-100/[0.04] p-1 text-xs">
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-[1fr_280px]">
              <label className="flex flex-col gap-2">
                <span className="text-sm text-paper-300">Export JSON</span>
                <textarea
                  className="textarea min-h-[420px] font-mono text-xs leading-5"
                  value={importJson}
                  onChange={(event) => {
              ...
              <div className="space-y-3">
                <label className="block rounded-lg border border-paper-100/10 bg-paper-100/[0.03] p-3 text-sm text-paper-300">
                  <span className="block font-medium text-paper-100">Load .json file</span>
                  <input
                    type="file"
                    accept="application/json,.json"
                    className="mt-3 block w-full text-xs text-paper-400 file:mr-3 file:rounded-md file:border-0 file:bg-paper-100/10 file:px-3 file:py-2 file:text-paper-100"
                    onChange={(event) => {
        ...
        <section className="surface rounded-xl p-5">
          ...
          >
            ...
            <label className="flex flex-col gap-2">
              ...
              <textarea
                className="textarea min-h-[420px] font-mono text-sm leading-6"
                value={draft.body}
                onChange={(event) =>
