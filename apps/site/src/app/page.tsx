...
const routingRows = [
  ['critical', 'enterprise checkout', 'Plan upgrade breaks payment confirmation', 'Linear draft ready'],
  ['expedite', 'production latency', 'Reports loading in 18.4 seconds for EU accounts', 'owner suggested'],
  ['watch', 'docs gap', 'Webhook retry behavior unclear after failed delivery', 'KB match missing'],
  ['none', 'product idea', 'Expose saved dashboard views to teammates', 'kept in queue'],
];
    ...
    <main className="page">
      ...
      <section className="shell hero">
        <div className="hero-copy">
          ...
          <h1>
            Customer signal, turned into accountable product work.
          </h1>
          <p className="lead">
            Triage gives product, support, and engineering teams one owned path from
            widget submission to triage, escalation, internal handling, and external
            issue handoff.
          </p>
        ...
        <div className="hero-visual" aria-label="Triage product workflow preview">
      ...
      <section className="shell intro-band" id="product">
        ...
        <p>
          The product is built for teams that need context before tickets become engineering work:
          who sent it, where it happened, what account it affects, whether docs can answer it,
          and which escalation rule made it urgent.
        </p>
      </section>
      ...
      <section className="shell routing" id="routing">
        <div className="routing-copy">
          ...
          <p>
            Use AI classification for the first pass, then keep routing decisions explicit.
            A support lead can inspect why feedback was marked critical, adjust the rule,
            and leave the automation in place.
          </p>
        </div>
      ...
      <section className="proof" id="proof">
        <div className="shell proof-inner">
          <div className="proof-panel">
            ...
            <h2>Cleaner queues. Better handoffs. Fewer orphaned customer reports.</h2>
            <p>
              Triage is opinionated about the handoff moment. Engineering should receive
              structured work, not vague screenshots. Support should keep ownership when a
              conversation does not need a backlog issue.
            </p>
          </div>
      ...
      <section className="shell deploy" id="deploy">
        <div className="deploy-copy">
          ...
          <h2>Install the widget with one script tag. Own everything behind it.</h2>
          <p>
            Host the dashboard and backend yourself, connect your storage and provider keys,
            then place the embed wherever customers already are.
          </p>
          <a
...
function Icon({ name }: { name: string }) {
  ...
  if (name === 'mark') {
    return (
      <svg {...common}>
        <path d="M5 7h14M7 12h10M9.5 17h5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  if (name === 'route') {
    return (
      <svg {...common}>
        <path d="M6 5h4a4 4 0 010 8H8a3 3 0 000 6h10" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
        <path d="M17 16l3 3-3 3M5 5h.01" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
  ...
  if (name === 'handoff') {
    return (
      <svg {...common}>
        <path d="M4 7h7v10H4zM13 5h7v14h-7zM8 12h8" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
  ...
  if (name === 'kb') {
    return (
      <svg {...common}>
        <path d="M6 4.5A3.5 3.5 0 019.5 1H18v18H9.5A3.5 3.5 0 006 22.5v-18z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
        <path d="M10 7h5M10 11h5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  if (name === 'lock') {
    return (
      <svg {...common}>
        <path d="M7 10V8a5 5 0 0110 0v2M6 10h12v10H6z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
  ...
  return (
    <svg {...common}>
      <path d="M4 6h16v12H4zM7 9h5M7 13h10M15 9h2" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
